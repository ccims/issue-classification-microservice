...
RATE_MODIFIER_MAP = { 's': lambda n: n,'m': lambda n: n / 60,'h': lambda n: n / 60 / 60 }
TIME_UNITS = (('day', 60 * 60 * 24, lambda n: format(n, '.2f')), ('hour', 60 * 60, lambda n: format(n, '.2f')), ('minute', 60, lambda n: format(n, '.2f')), ('second', 1, lambda n: format(n, '.2f')))
ZERO = timedelta(0)
...
def remaining(start, ends_in, now=..., relative=...):
...
def humanize_seconds(secs, prefix=..., sep=..., now=..., microseconds=...):
...
class ffwd:
    ...
    def __init__(self, year=..., month=..., weeks=..., weekday=..., day=..., hour=..., minute=..., second=..., microsecond=..., **kwargs) -> None:
        ...
