...
def conf(state, with_defaults=..., **kwargs):
    """List configuration."""
    ...
...
def revoke(state, task_id, terminate=..., signal=..., **kwargs):
...
def time_limit(state, task_name=..., hard=..., soft=..., **kwargs):
...
@control_command()
def election(state, id, topic, action=..., **kwargs):
    """Hold election.
...
def hello(state, from_node, revoked=..., **kwargs):
...
def registered(state, taskinfoitems=..., builtins=..., **kwargs):
...
@inspect_command(default_timeout=60, args=[('type', str), ('num', int), ('max_depth', int)], signature='[object_type=Request] [num=200 [max_depth=10]]')
def objgraph(state, num=..., max_depth=..., type=...):
...
def add_consumer(state, queue, exchange=..., exchange_type=..., routing_key=..., **options):
